namespace VSCC.Roll20.Macros.Math
{
    ...
    public class MacroActionFloor : MacroAction
    {
        private readonly MacroAction[] _backend = new MacroAction[1];

        public override string Name => this.Translate("Macro_MathFloor_Name");
        ...
        public override string CreateFullInnerText() => this.Translate("Macro_MathFloor_FullInnerText", this.Params[0].CreateFullInnerText());

        public override IEnumerable<Inline> CreateInnerText()
        {
            yield return new Run(this.Translate("Macro_MathFloor_Text_0"));
            yield return new Hyperlink(new Run()) { Tag = 0 };
            yield return new Run(this.Translate("Macro_MathFloor_Text_1"));
        }

        public override void Deserialize(BinaryReader br) => this.Params[0] = MacroSerializer.ReadMacroAction(br);

        public override object Execute(Macro m, List<string> errors) => (int)Math.Floor((float)this.Params[0].Execute(m, errors));
