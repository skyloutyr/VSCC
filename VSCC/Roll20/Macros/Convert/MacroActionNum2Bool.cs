namespace VSCC.Roll20.Macros.Numbers
{
    ...
    public class MacroActionNum2Bool : MacroAction
    {
        private readonly MacroAction[] _backend = new MacroAction[1];

        public override string Name => this.Translate("Macro_ConvNum2Bool_Name");
        ...
        public override string CreateFullInnerText() => this.Translate("Macro_ConvNum2Bool_FullInnerText", this.Params[0].CreateFullInnerText());

        public override IEnumerable<Inline> CreateInnerText()
        {
            yield return new Hyperlink(new Run()) { Tag = 0 };
            yield return new Run(this.Translate("Macro_ConvNum2Bool_Text_0"));
        }

        public override void Deserialize(BinaryReader br) => this.Params[0] = MacroSerializer.ReadMacroAction(br);

        public override object Execute(Macro m, List<string> errors) => (int)this._backend[0].Execute(m, errors) > 0;
