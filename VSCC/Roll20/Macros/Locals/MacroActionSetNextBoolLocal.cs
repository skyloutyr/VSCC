namespace VSCC.Roll20.Macros.Locals
{
    ...
    public class MacroActionSetNextBoolLocal : MacroAction
    {
        private readonly MacroAction[] _backend = new MacroAction[2];

        public override string Name => this.Translate("Macro_LocalSetNextB_Name");

        public override string Category => this.Translate("Macro_Category_Locals");

        public override MacroAction[] Params => this._backend;

        public override Type[] ParamTypes => new Type[] { typeof(string), typeof(bool) };
        ...
        public override string CreateFullInnerText() => this.Translate("Macro_LocalSetNextB_FullInnerText", this.Params[0].CreateFullInnerText(), this.Params[1].CreateFullInnerText());
        public override IEnumerable<Inline> CreateInnerText()
        {
            yield return new Run(this.Translate("Macro_LocalSetNextB_Text_0"));
            yield return new Hyperlink(new Run()) { Tag = 0 };
            yield return new Run(this.Translate("Macro_LocalSetNextB_Text_1"));
        ...
        public override object Execute(Macro m, List<string> errors)
        {
            Macro.InbetweenMacroActions.Add((macro, errs) =>
            {
                ...
                if (macro.BoolLocals.ContainsKey(name))
                {
                    macro.BoolLocals[name] = new Tuple<bool, bool>((bool)this.Params[1].Execute(macro, errs), macro.BoolLocals[name].Item2);
                }
            });
