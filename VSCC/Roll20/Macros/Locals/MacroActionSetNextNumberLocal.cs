namespace VSCC.Roll20.Macros.Locals
{
    ...
    public class MacroActionSetNextNumberLocal : MacroAction
    {
        private readonly MacroAction[] _backend = new MacroAction[2];

        public override string Name => this.Translate("Macro_LocalSetNextN_Name");
        ...
        public override string CreateFullInnerText() => this.Translate("Macro_LocalSetNextN_FullInnerText", this.Params[0].CreateFullInnerText(), this.Params[1].CreateFullInnerText());
        public override IEnumerable<Inline> CreateInnerText()
        {
            yield return new Run(this.Translate("Macro_LocalSetNextN_Text_0"));
            yield return new Hyperlink(new Run()) { Tag = 0 };
            yield return new Run(this.Translate("Macro_LocalSetNextN_Text_1"));
        ...
        public override object Execute(Macro m, List<string> errors)
        {
            Macro.InbetweenMacroActions.Add((macro, errs) =>
            {
                ...
                if (macro.NumberLocals.ContainsKey(name))
                {
                    macro.NumberLocals[name] = new Tuple<int, int>((int)this.Params[1].Execute(macro, errs), macro.NumberLocals[name].Item2);
                }
            });
