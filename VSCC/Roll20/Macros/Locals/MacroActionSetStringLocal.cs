namespace VSCC.Roll20.Macros.Locals
{
    ...
    public class MacroActionSetStringLocal : MacroAction
    {
        private readonly MacroAction[] _backend = new MacroAction[2];

        public override string Name => this.Translate("Macro_LocalSetS_Name");
        ...
        public override string CreateFullInnerText() => this.Translate("Macro_LocalSetS_FullInnerText", this.Params[0].CreateFullInnerText(), this.Params[1].CreateFullInnerText());

        public override IEnumerable<Inline> CreateInnerText()
        {
            yield return new Run(this.Translate("Macro_LocalSetS_Text_0"));
            yield return new Hyperlink(new Run()) { Tag = 0 };
            yield return new Run(this.Translate("Macro_LocalSetS_Text_1"));
        ...
        public override object Execute(Macro m, List<string> errors)
        {
            ...
            if (m.StringLocals.ContainsKey(name))
            {
                m.StringLocals[name] = new Tuple<string, string>(this.Params[1].Execute(m, errors).ToString(), m.StringLocals[name].Item2);
            }
